...
// Autonomous R9 swap settings/state.
...
function r9GetLogDb() {
  try {
    ...
    let logDb = game.db.log10();
    if (logDb.toNumber !== undefined)
      return logDb.toNumber();

    return parseFloat("" + logDb);
  } catch (e) {
    return null;
  }
}
...
function r9StartSwap(manual = false) {
  ...
  // Save existing R1/R2/R3 levels so only the temporary swap purchases are removed later.
  r9SwapPrevBoostLevels = [
    game.researchUpgrades[0]?.level ?? 0,
    game.researchUpgrades[1]?.level ?? 0,
  ...
  // Step 2: buy max R1, then max R2, then max R3.
  for (let i = 0; i < 3; i++) {
    let upgrade = game.researchUpgrades[i];
...
class UIutils {
  ...
  static createLatexClickButton(text, onClicked) {
...
var tick = (elapsedTime, multiplier) => {
  ...
  r9ObserveMainGameGrowth(elapsedTime);
